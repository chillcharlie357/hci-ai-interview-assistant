...
- 面试中：数字人面试官按问题逐题提问，并记录候选人回答、用时和基础表达指标。
- 面试后：基于问题、回答和事件日志生成结构化智能面试纪要。
...
本产品 MVP 希望通过问题生成、数字人提问、回答记录和 LLM 纪要生成，先跑通最小可用闭环。视频会议、屏幕共享、OCR、非语言分析和真实数字人渲染均作为后续增强能力。
...
3. 数字人面试官展示并朗读当前问题。
4. 用户输入或粘贴候选人回答。
5. 系统记录回答文本、回答用时、字数和填充词数量。
6. 系统进入下一题并持续记录事件。
7. 所有问题结束后生成面试纪要。

### 6.2 面试后纪要流程

1. 面试结束，系统保存问题、回答和事件日志。
2. 系统按问题维度整理回答记录。
3. 系统统计基础指标，如回答用时、字数、填充词数量。
4. 系统生成建议追问和待人工确认项。
5. LLM 或规则模板基于结构化证据生成智能纪要。
6. 用户查看纪要、问题、回答和待确认项。
...
- 问题生成：MVP 使用 Python 规则模板，后续可替换为 LLM。
...
- 纪要生成：MVP 使用 Python 规则模板，后续可接入 LLM 生成更自然的摘要和追问建议。
...
后端 / 核心逻辑：Python
前端 UI：TypeScript
问题生成：Python 规则模板，参考 FoloUp / Flo-Interviewer / CVQuest
数字人：TypeScript UI + 前端 TTS + 静态头像
回答记录：文本输入 + 回答用时 + 字数 + 填充词统计
面试后纪要：Python 规则模板生成 Markdown
```
...
这个产品的 MVP 不是视频分析系统，而是一个最小 AI 面试闭环：面试前基于简历和 JD 生成问题，面试中由数字人逐题提问并记录回答，面试后基于问题和回答生成可复盘、可追溯的智能纪要。
