...
本产品 MVP 希望通过问题生成、数字人主持式提问、回答记录、浏览器端实时摄像头观察和 LLM 纪要增强，先跑通最小可用闭环。视频会议、屏幕共享、OCR 和真实数字人渲染仍作为后续增强能力。
...
- 支持数字人面试官以会议参与者卡片和前端 TTS 的方式逐题自动提问，面试端以双方字幕流呈现问答。
...
4. 数字人面试官以会议卡片形式展示，自动朗读当前问题，并在字幕区显示 AI 提问。
5. 候选人口头回答，语音转文字结果以候选人字幕展示；浏览器不支持语音识别时保留文本字幕 fallback。
6. 候选人结束回答后，系统记录回答文本、回答用时、字数、填充词数量和摄像头事件。
7. 系统进入下一题，数字人自动继续提问并持续记录事件。
8. 所有问题结束后生成面试纪要。
...
- 在面试会议区展示 AI 面试官卡片。
- 使用前端 TTS 自动朗读当前问题，问题切换后自动播报下一题。
- 展示准备中、提问中、等待回答、已结束和不支持状态。
- 在字幕区展示 AI 提问和候选人回答，主流程不使用“点击朗读”和“提交回答”表单按钮。
- 后续可替换为 LiveKit Agents、Pipecat 或 LiveTalking。

MVP 推荐组件：

- Web Speech API：前端 TTS 的 MVP 实现。
- 静态头像 + 说话动画：作为 MVP 数字人占位。
