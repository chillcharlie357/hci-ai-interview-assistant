...
- 支持招聘端查看生成的完整测试题目，包含主问题、追问建议和观察点。
- 支持按报告权限下载 Markdown 面试结果。
- 支持独立控制面板页面，展示统计数据、最近面试、快捷入口。
- 支持 Mock 模式快速创建测试面试，跳过简历解析流程。
- 支持报告页雷达图可视化展示软技能分析。
...
2. 系统生成结构化问题列表，并在招聘端展示完整测试题目、追问建议和观察点。
...
6. 用户查看纪要、问题、回答和待确认项，并可按权限下载 Markdown 面试结果。
...
- 预录视频头像 + 状态驱动切换（说话/微笑/点头/摇头）：作为 MVP 数字人占位。
...
- 保留证据链引用。
- 标记低置信度和待人工确认项。
- 招聘端始终可下载 Markdown 纪要；候选人端仅在报告配置为双方可见时可下载。
